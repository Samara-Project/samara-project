        ...
        <section id='about' className='container mx-auto py-12 md:py-16 lg:py-20 grid grid-cols-2 lg:grid-cols-3 gap-10'>
            {experienceData.sort((a, b) => a.order - b.order).map((experience, index) => (
                <div key={experience?.title + index} className='flex flex-col items-center w-full justify-between'>
                    ...
                    <Link target='_blank' href={experience?.ctaLink} className='mt-4 bg-primary hover:bg-primary/95 transition-colors rounded-full px-8 py-2 lg:py-2.5 text-white font-bold text-lg sm:text-xl lg:text-xl xl:text-2xl inline-block shadow-md hover:shadow-lg'>
                        {experience?.ctaText}
                    </Link>
                </div>
            ))}
